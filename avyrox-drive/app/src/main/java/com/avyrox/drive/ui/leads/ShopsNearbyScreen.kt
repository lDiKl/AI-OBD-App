package com.avyrox.drive.ui.leads
...
private sealed class ServicesUiState {
    object Idle : ServicesUiState()
    object Loading : ServicesUiState()
    data class Loaded(val shops: List<ShopDto>) : ServicesUiState()
...
fun ShopsNearbyScreen(
    ...
    var uiState by remember { mutableStateOf<ServicesUiState>(ServicesUiState.Idle) }
    ...
    fun loadShops() {
        scope.launch {
            uiState = ServicesUiState.Loading
            try {
                val (lat, lng) = getCurrentLocation(context)
                leadsRepository.getNearbyShops(lat, lng).fold(
                    onSuccess = { uiState = ServicesUiState.Loaded(it) },
                    onFailure = { uiState = ServicesUiState.Error(it.message ?: "Failed to load shops") },
                )
            } catch (e: CancellationException) {
                throw e  // never swallow CancellationException
            } catch (e: Exception) {
                uiState = ServicesUiState.Error(e.message ?: "Could not get location")
            }
        }
    }
    ...
    val permLauncher = rememberLauncherForActivityResult(
        ...
        else uiState = ServicesUiState.Error("Location permission is required to find nearby services")
    }
    ...
    if (successDialog) {
        AlertDialog(
            ...
            text = { Text("The service will review your diagnostic and send you a quote. Check \"My Leads\" for updates.") },
            confirmButton = {
                TextButton(onClick = {
                    successDialog = false
                    onLeadSent()
    ...
    Scaffold(
        ...
        Box(modifier = Modifier.fillMaxSize().padding(padding)) {
            when (val s = uiState) {
                is ServicesUiState.Idle ->
                    CenterLoading("Requesting location…")
                is ServicesUiState.Loading ->
                    CenterLoading("Finding nearby services…")
                ...
                is ServicesUiState.Loaded -> {
                    if (s.shops.isEmpty()) {
                        Column(
                            ...
                            Text("No services found nearby", style = MaterialTheme.typography.titleMedium)
                            Spacer(Modifier.height(8.dp))
                            Text(
                                "No registered services within 50 km.",
                        ...
                        Column(modifier = Modifier.fillMaxSize()) {
                            Text(
                                "${s.shops.size} service(s) found within 50 km",
